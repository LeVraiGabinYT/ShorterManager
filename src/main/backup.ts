...
function importReplace(data: BackupData): BackupImportResult {
  ...
  const txn = db.transaction(() => {
    ...
    const insertTaskType = db.prepare(
      `INSERT INTO task_types (id, name, emoji, position, created_at)
       VALUES (@id, @name, @emoji, @position, @createdAt)`
    )
    ...
    const insertTask = db.prepare(
      `INSERT INTO tasks
         (id, title, emoji, due_date, due_time, status, created_at, updated_at)
       VALUES
         (@id, @title, @emoji, @dueDate, @dueTime, @status, @createdAt, @updatedAt)`
    )
